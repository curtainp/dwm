...
# 更新状态栏
refresh() {
    _music='';_wifi='';_cpu='';_mem='';_date='';_vol=''                  # 重置所有模块的状态为空
    source $tempfile                                                     # 从 temp 文件中读取模块的状态
    xsetroot -name "$_music$_wifi$_cpu$_mem$_date$_vol"                  # 更新状态栏
}
...
cron() {
    ...
    while true; do
        ...
        [ $i -lt 30 ] && to=(wifi cpu mem date vol)            # 前 30秒  更新所有模块
...
case $1 in
    ...
    updateall|check) update music wifi cpu mem date vol ;;
