...
# 更新状态栏
refresh() {
    _music='';_wifi='';_bluetooth='';_cpu='';_mem='';_date='';_vol=''                  # 重置所有模块的状态为空
    source $tempfile                                                     # 从 temp 文件中读取模块的状态
    xsetroot -name "$_music$_wifi$_bluetooth$_cpu$_mem$_date$_vol"                  # 更新状态栏
}
...
cron() {
    ...
    while true; do
        ...
        [ $((i % 20)) -eq 0 ]  && to=(${to[@]} cpu mem vol bluetooth)              # 每 20秒  更新 cpu mem vol icons
        # [ $((i % 300)) -eq 0 ] && to=(${to[@]} bat)                      # 每 300秒 更新 bat
        [ $((i % 5)) -eq 0 ]   && to=(${to[@]} date music)               # 每 5秒   更新 date
        [ $i -lt 30 ] && to=(wifi cpu mem date vol bluetooth)            # 前 30秒  更新所有模块
...
case $1 in
    ...
    updateall|check) update music wifi bluetooth cpu mem date vol ;;
